; (function () {
	...
	function winput(args) { // a winput (widget input) lets user input a string or a number
		...
		args = args || {}
	    var cvs = canvas.get_selected()
	    var attrs = {pos:cvs.caption_anchor, text:undefined, prompt:'', width:100, height:20, type:'numeric', number:null, disabled:false}
	    for (var a in attrs) {
	    	if (args[a] !== undefined) {
	    		attrs[a] = args[a]
	    		delete args[a]
		...
		if (attrs.type != 'numeric' && attrs.type != 'string') throw new Error('A winput type must be "numeric" or "string".')
		...
		if (attrs.bind === undefined) {
			while (true) {
				...
				if (attrs.type == 'numeric') {			
					var ok = wparse()
					if (!ok) alert('Numeric input error')
					else return attrs.number
	...
	function slider(args) {
		...
		attrs.jslider = $( '#'+attrs._id ).slider({orientation:o, range:"min",
			...
			change: function (evt) { if (!attrs.disabled && evt.originalEvent !== undefined) attrs.bind(cslider) },
			slide:  function (evt) { if (!attrs.disabled && evt.originalEvent !== undefined) attrs.bind(cslider) }
		})
	    ...
	    var cmenu = { // this structure implements a JavaScript "closure", essential for menu to work
			...
			set choices(value) {
				...
				for (var i=0; i<value.length; i++) {
					$(attrs.jmenu).append($("<option></option>").text(value[i]))
				}
			},
