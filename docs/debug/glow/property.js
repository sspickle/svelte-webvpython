	...
	var property = {
		declare: function (proto, propertyMap) {
			$.each(propertyMap, function (name, definition) {
				...
				if (definition.enumerable === undefined) definition.enumerable = !internal;
				if (definition.type === undefined && !definition.get && !internal) {
					var tp = typeof definition.value;
					if (tp === 'number') definition.type = property.check_number;
					else if (tp === 'string') definition.type = property.check_string;
					...
					else if (definition.value instanceof vec) definition.type = property.check_vec;
				}

				if (
					(definition.readonly && definition.set) ||
				...
				if (definition.get) {
					Object.defineProperty(proto, name, {
						...
						get: definition.get,
						set: definition.set || readOnlyError
					});
				} else {
					...
					if (!definition.type && !definition.onchanged)
						...
						if (definition.set) prop.set = definition.set;
						else if (definition.onchanged && definition.type)
							prop.set = function (val) {
								var old = this[internalName];
								this[internalName] = definition.type.call(this, val, definition);
								definition.onchanged.call(this, old);
							};
						...
						Object.defineProperty(proto, internalName, {
							enumerable: false,
							writable: true,
							value: definition.value
						});
		...
		check_attributeVector: function check_attributeVector(v, def) {
			if (!(v instanceof vec)) {
				throw new Error("Property '" + def.name + "' must be a vector.");
			}
		...
		check_number: function check_number(v, def) {
			return v;
		},
		check_string: function check_string(v, def) {
			return v;
		}
	};
